...
import sys
import os
#path = "E:\\IAA3DWeaveProject\\parameterisedTextile\\"
...
def GenerateTextile(numXYarns, numWefts, warpSpacing, weftSpacing, warpHeight, warpWidth, weftHeight, weftWidth, binderHeight, binderWidth, warpRatio, binderRatio, length, width, height, binderYarns, numWeftLayers, numWarpLayers, numBinderLayers):
	...
	weftSpacing = weftSpacing + (numBinderLayers-1)*binderHeight
	...
	for z in range(NumBinderYarns):
		...
		for x in range( numWefts ):
			...
			ind = z/binderRatio
			BinderIndex = int(warpRatio + (ind * repeat) + z%binderRatio)
	...
	Textile.SetFibresPerYarn(BINDER, 6000)
	...
	domain = CDomainPlanes(XYZ(0, 0, -binderHeight*1.1), XYZ(length, width, height))
	
	x_translation = length/2
	y_translation = width/2
	z_translation = (height - (binderHeight*1.1))/2
	
	Textile.Translate(XYZ(-x_translation, -y_translation, -z_translation))
	domain.Translate(XYZ(-x_translation, -y_translation, -z_translation))
	...
	voxelSize = 0.04
	#SaveToXML(r"E:\\IAA3DWeaveProject\\parameterisedTextile\\ptextile.tg3", Textile.GetName(), OUTPUT_STANDARD)
	...
	while numVoxels >= 800000:
		voxelSize += 0.05*voxelSize
		XVoxNum = int(length / voxelSize)
		YVoxNum = int(width  / voxelSize)
		ZVoxNum = int(height / voxelSize)
		numVoxels = XVoxNum * YVoxNum * ZVoxNum
	
	# file = open("voxels.txt", "a")
	# string=str(XVoxNum) + " " + str(YVoxNum)+ " " + str(ZVoxNum)+ " " + str(numVoxels) + "\n"
	# file.write(string)
	# file.close()
	# #Get input file name
...
def SaveMesh(Textile, XVoxNum, YVoxNum, ZVoxNum, input):
	...
	Mesh = CRectangularVoxelMesh('CBendingPeriodicBoundaries')
	Mesh.SaveVoxelMesh(Textile, os.getcwd() + "/" + "optim_" + results_id, XVoxNum, YVoxNum, ZVoxNum, bool(1), bool(1), 5, 0 )
	
	return

if __name__ == '__main__':
	path = "/gpfs01/home/emxghs/AbaqusRunParallel"
	#print(sys.argv)
	...
	file=open("binderpattern_"+ results_id +".dat", "r")
	...
	#print(numXYarns, numWefts, warpSpacing, weftSpacing, warpHeight, warpWidth, weftHeight, weftWidth, binderHeight, binderWidth, warpRatio, binderRatio, length, width, height, binderYarns, numWeftLayers, numWarpLayers, numBinderLayers)
	...
	try:
		...
		file = open("ArealDensity_" + results_id + ".txt", "a")
