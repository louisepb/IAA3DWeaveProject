...
%write to .dat file
bpatternfile = sprintf("binderpattern_%d_%d_%d_%d_%d.dat", input)
fileID=fopen(bpatternfile, "a");
format="";
...
format3 = " %d %d %d ";
[cmdLine1, errmsg1] = sprintf('python3 parameterisedTextile.py ' + format1, string1 );
...
status;
cmdout2
ArealDensityFormat = sprintf("ArealDensity_%d_%d_%d_%d_%d.txt", input)
if status ~= 0
	ArealDensityFile = fopen(ArealDensityFormat, "w");
	fprintf(ArealDensityFile, string(0));
	fclose(ArealDensityFile);
	ArealDensity = 0;
else
	ArealDensity = dlmread(ArealDensityFormat);
