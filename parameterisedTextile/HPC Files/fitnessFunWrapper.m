...
[status, cmdout] = system(char("sbatch fitnessFun.sh " + strcat(num2str(input)) + '  ' + strcat(num2str(ArealDensity )) ));
cmdout
status

resultsfileafter=isfile(sprintf("optim_%d_%d_%d_%d_%d_results.txt", input))
while resultsfileafter==0
	resultsfileafter=isfile(sprintf("optim_%d_%d_%d_%d_%d_results.txt", input))
	pause(5)
end
