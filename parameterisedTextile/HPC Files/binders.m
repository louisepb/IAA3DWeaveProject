...
format3 = " %d %d %d ";
[cmdLine1, errmsg1] = sprintf('sbatch parameterisedTextile.sh ' + format1, string1 );
[cmdLine3, errmsg3] = sprintf(format3, string3);

cmdLine = cmdLine1 + cmdLine3 + strcat(num2str(input));
file=fopen("command.dat", "a");
fprintf(file, cmdLine);
fclose(file);

size1=dir('ArealDensity.txt');
filesize1=size1.bytes;
[status, cmdout2] = system(cmdLine, '-echo');

size2 = dir('ArealDensity.txt');
filesize2=size2.bytes;
