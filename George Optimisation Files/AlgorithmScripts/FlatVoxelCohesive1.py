...
import os
os.chdir('c:\users\emxghs\desktop')
import sys
sys.path.append('c:\users\emxghs\desktop')
...
nly=4
planepos=nly-nbl #change on other files
from CheckBinderPaths import *
a=CheckBinderPaths3NoBifurcation(planepos, nbl)
b=CheckBinderPaths1NoBifurcation(nly, nbl)
if (a != 0 or b != 0):
	file=open('fitfun1.dat', 'a')
	sum=((a*100) + (b*100))
	file.write(str(sum) + ' \n')
	file.close()
	
	fil2=open("bool.txt", "w+")
	fil2.write("False")
	fil2.close()
else:
	fil2=open("bool.txt", "w+")
	...
	threadNumber=1
	ModelName= "weave_" + str(threadNumber) + "_" + str(NumXVoxels)
	...
	rv.SaveVoxelMeshg(t, FileName, NumXVoxels, NumYVoxels, NumZVoxels*2, True, True, True, NO_BOUNDARY_CONDITIONS, 0)


	from WriteInput import *

	WriteToInputExplicit(FileName)
